...
def make_mfcc(file_path, save_path):
    ...
    if not np.all(np.isfinite(audio_data)):
        print(f"Warning: Non-finite values found in {file_path}. Skipping this file.")
        return
    ...
    if max_val == 0:
        print(f"Warning: Max value is zero in {file_path}. Skipping this file.")
        return
    audio_data /= max_val
